...
#         return model
    if 'efficient' in cfg.model.name:
        ...
        if 'b5' in cfg.model.name:
            model._fc = torch.nn.Linear(2048, cfg.model.n_output)
        elif 'b4' in cfg.model.name:
            model._fc = torch.nn.Linear(1792, cfg.model.n_output)
        elif 'b3' in cfg.model.name:
            model._fc = torch.nn.Linear(1536, cfg.model.n_output)
        elif 'b2' in cfg.model.name:
            model._fc = torch.nn.Linear(1408, cfg.model.n_output)
        elif 'b1' in cfg.model.name:
